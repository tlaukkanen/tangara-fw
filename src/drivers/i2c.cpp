...
esp_err_t init_i2c(void) {
  ...
  i2c_config_t config2 = {
      .mode = I2C_MODE_MASTER,
      .sda_io_num = kI2C2SdaPin,
      .scl_io_num = kI2C2SclPin,
      .sda_pullup_en = GPIO_PULLUP_ENABLE,
      .scl_pullup_en = GPIO_PULLUP_ENABLE,
      .master =
          {
              .clk_speed = kI2C2ClkSpeed,
          },
      // No requirements for the clock.
      .clk_flags = 0,
  };

  if (esp_err_t err = i2c_param_config(kI2C2Port, &config2)) {
    return err;
  }
  if (esp_err_t err = i2c_driver_install(kI2C2Port, config2.mode, 0, 0, 0)) {
    return err;
  }
